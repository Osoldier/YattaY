...
public class LevelRenderer {
	...
	private boolean inView(Camera2d c, int x, int y) {
		 return (x > c.getPosition().x-Block.SIZE/2 && x < c.getPosition().x + Editor.getWindow().getPixWidth()+Block.SIZE/2) && (y > c.getPosition().y-Block.SIZE/2 && y < c.getPosition().y + Editor.getWindow().getPixHeight()+Block.SIZE/2);
	}
